...
<body <?php geniewizards_body_class(); ?>>
<header id="header">
  <div class="top_links">
    <div class="container">
      <div class="language">
      ...
      <?php if(geniewizards_welcome_message()){ ?>
      <p class="welcome-message"><?php echo geniewizards_welcome_message(); ?></p>
      <?php } ?>
      <ul>
        ...
        <li class="first logged"> <span><?php echo sprintf(__('Hi %s', genieWIZARDS_THEME_FOLDER), osc_logged_user_name() . '!'); ?> </span> &#10072; <strong><a href="<?php echo osc_user_dashboard_url(); ?>">
  ...
  <?php 
	if( osc_is_home_page() ) {
		if(osc_get_preference('show_banner', 'geniewizards_theme')=='1'){
      ...
      <div class="container">
        ...
        <div class="main-search">
          <div class="form-filters">
            <div class="row">
              <?php $showCountry  = (osc_get_preference('show_search_country', 'geniewizards_theme') == '1') ? true : false; ?>
              <div class="col-md-<?php echo ($showCountry)? '3' : '4'; ?>">
                <div class="cell">
                  <input type="text" name="sPattern" id="query" class="input-text" value="" placeholder="<?php echo osc_esc_html(__(osc_get_preference('keyword_placeholder', 'geniewizards_theme'), genieWIZARDS_THEME_FOLDER)); ?>" />
                </div>
              </div>
              <div class="col-md-2">
                <?php  if ( osc_count_categories() ) { ?>
                <div class="cell selector">
                  <?php osc_categories_select('sCategory', null, osc_esc_html(__('Select a category', genieWIZARDS_THEME_FOLDER))) ; ?>
                </div>
                <?php  } ?>
              </div>
              <?php if($showCountry) { ?>
              <div class="col-md-2">
                <div class="cell selector">
                  <?php geniewizards_countries_select('sCountry', 'sCountry', __('Select a country', genieWIZARDS_THEME_FOLDER));?>
                </div>
              </div>
              <?php } ?>
              <div class="col-md-2">
                <div class="cell selector">
                  <?php geniewizards_regions_select('sRegion', 'sRegion', __('Select a region', genieWIZARDS_THEME_FOLDER)) ; ?>
                </div>
              </div>
              <div class="col-md-2">
                <div class="cell selector">
                  <?php geniewizards_cities_select('sCity', 'sCity', __('Select a city', genieWIZARDS_THEME_FOLDER)) ; ?>
                </div>
              </div>
              <div class="col-md-<?php echo ($showCountry)? '1' : '2'; ?>">
                <div class="cell reset-padding">
                  <button  class="btn btn-success btn_search"><i class="fa fa-search"></i> <span <?php echo ($showCountry)? '' : 'class="showLabel"'; ?>><?php echo osc_esc_html(__("Search", genieWIZARDS_THEME_FOLDER));?></span> </button>
                </div>
              </div>
            </div>
          </div>
...
<div class="container">
<?php if( osc_get_preference('header-728x90', 'geniewizards_theme') !=""){ ?>
<div class="ads_header ads-headers"> <?php echo osc_get_preference('header-728x90', 'geniewizards_theme'); ?> </div>
<?php } ?>
<div id="main">
