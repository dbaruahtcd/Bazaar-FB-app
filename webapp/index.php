<?php
    ...
    if( CLI ) {
        ...
        if(Params::getParam('page')=='upgrade') {
            require_once(osc_lib_path() . 'genie/upgrade-funcs.php');
            exit(1);
    ...
    if( file_exists(ABS_PATH . '.maintenance') ) {
        if(!osc_is_admin_user_logged_in()) {
            ...
            if(file_exists(WebThemes::newInstance()->getCurrentThemePath().'maintenance.php')) {
                ...
                require_once LIB_PATH . 'genie/helpers/hErrors.php';
    ...
    switch( Params::getParam('page') )
    {
        case ('cron'):      // cron system
                            ...
                            require_once(osc_lib_path() . 'genie/cron.php');
        break;
        case ('user'):      // user pages (with security)
                            ...
                            || Params::getParam('action')=='pub_profile') {
                                require_once(osc_lib_path() . 'genie/controller/user-non-secure.php');
                                $do = new CWebUserNonSecure();
                                $do->doModel();
                            } else {
                                require_once(osc_lib_path() . 'genie/controller/user.php');
                                $do = new CWebUser();
                                $do->doModel();
                            }
        break;
        case ('item'):      // item pages
                            require_once(osc_lib_path() . 'genie/controller/item.php');
                            $do = new CWebItem();
                            $do->doModel();
        break;
        case ('search'):    // search pages
                            require_once(osc_lib_path() . 'genie/controller/search.php');
                            $do = new CWebSearch();
                            $do->doModel();
        break;
        case ('page'):      // static pages
                            require_once(osc_lib_path() . 'genie/controller/page.php');
                            $do = new CWebPage();
                            $do->doModel();
        break;
        case ('register'):  // register page
                            require_once(osc_lib_path() . 'genie/controller/register.php');
                            $do = new CWebRegister();
                            $do->doModel();
        break;
        case ('ajax'):      // ajax
                            require_once(osc_lib_path() . 'genie/controller/ajax.php');
                            $do = new CWebAjax();
                            $do->doModel();
        break;
        case ('login'):     // login page
                            require_once(osc_lib_path() . 'genie/controller/login.php');
                            $do = new CWebLogin();
                            $do->doModel();
        break;
        case ('language'):  // set language
                            require_once(osc_lib_path() . 'genie/controller/language.php');
                            $do = new CWebLanguage();
                            $do->doModel();
        break;
        case ('contact'):   //contact
                            require_once(osc_lib_path() . 'genie/controller/contact.php');
                            $do = new CWebContact();
                            $do->doModel();
        break;
        case ('custom'):   //contact
                            require_once(osc_lib_path() . 'genie/controller/custom.php');
                            $do = new CWebCustom();
                            $do->doModel();
        break;
        default:            // home and static pages that are mandatory...
                            require_once(osc_lib_path() . 'genie/controller/main.php');
                            $do = new CWebMain();
                            $do->doModel();
        break;
    }
